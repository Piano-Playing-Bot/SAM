...
void jar_xm_debug(jar_xm_context_t *ctx) {
    ...
    // DEBUG VARIABLES
    y += size; RL_DrawText(TextFormat("CUR TBL = %i", ctx->current_table_index),       x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("CUR PAT = %i", ctx->module.pattern_table[ctx->current_table_index]),   x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("POS JMP = %d", ctx->position_jump),             x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("JMP DST = %i", ctx->jump_dest),                 x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("PTN BRK = %d", ctx->pattern_break),             x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("CUR ROW = %i", ctx->current_row),               x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("JMP ROW = %i", ctx->jump_row),                  x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("ROW LCT = %i", ctx->row_loop_count),            x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("LCT     = %i", ctx->loop_count),                x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("MAX LCT = %i", ctx->max_loop_count),            x, y, size, RL_WHITE);
    x = size * 12; y = 0;

    y += size; RL_DrawText(TextFormat("CUR TCK = %i", ctx->current_tick),              x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("XTR TCK = %i", ctx->extra_ticks),               x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("TCK/ROW = %i", ctx->tempo),                     x, y, size, RL_ORANGE);
    y += size; RL_DrawText(TextFormat("SPL TCK = %f", ctx->remaining_samples_in_tick), x, y, size, RL_WHITE);
    y += size; RL_DrawText(TextFormat("GEN SPL = %i", ctx->generated_samples),         x, y, size, RL_WHITE);
    y += size * 7;
...
//            RL_DrawText(TextFormat("%02X", ctx->current_tick), i * size * 2, y - size, size, RL_WHITE);
            DrawRectangle(i * size * 2, y, size * 2, size, RL_RED);
            RL_DrawText(TextFormat("%02X", ctx->current_row), i * size * 2, y - size, size, RL_YELLOW);
        } else {
            DrawRectangle(i * size * 2, y, size * 2, size, RL_ORANGE);
        };
        RL_DrawText(TextFormat("%02X", ctx->module.pattern_table[i]), i * size * 2, y, size, RL_WHITE);
    };
    y += size;
    ...
    for (int j=(ctx->current_row - 14); j<(ctx->current_row + 15); j++) {
        ...
        if (j >=0 && j < (cur->num_rows)) {
            ...
            RL_DrawText(TextFormat("%02X",j), x, y, size, RL_WHITE);
            x += 2 * size;
            for(uint8_t i = 0; i < ctx->module.num_channels; i++) {
                if (j==(ctx->current_row)) {
                    DrawRectangle(x, y, 8 * size, size, RL_DARKGREEN);
                } else {
                    DrawRectangle(x, y, 8 * size, size, DARKRL_GRAY);
                };
                jar_xm_pattern_slot_t *s = cur->slots + j * ctx->module.num_channels + i;
           //     jar_xm_channel_context_t *ch = ctx->channels + i;
                if (s->note > 0) {RL_DrawText(TextFormat("%s%s", xm_note_chr(s->note), xm_octave_chr(s->note) ), x, y, size, RL_WHITE);} else {RL_DrawText("...", x, y, size, RL_GRAY);};
                if (s->instrument > 0) {
                    RL_DrawText(TextFormat("%02X", s->instrument), x + size * 2, y, size, RL_WHITE);
                    if (s->volume_column == 0) {
                        RL_DrawText(TextFormat("%02X", 64), x + size * 4, y, size, RL_YELLOW);
                    };
                } else {
                ...
                if (s->volume_column > 0) {RL_DrawText(TextFormat("%02X", (s->volume_column - 16)), x + size * 4, y, size, RL_WHITE);};
                if (s->effect_type > 0 || s->effect_param > 0) {RL_DrawText(TextFormat("%s%02X", xm_effect_chr(s->effect_type), s->effect_param), x + size * 6, y, size, RL_WHITE);};
                x += 9 * size;
            };
        };
    };
